...
export default function EditOwnerModal({ isOpen, onClose, owner, onSubmit }) {
  ...
  // Initialize form when owner prop changes
  useEffect(() => {
    if (owner) {
      setFormData({
        name: owner.name || '',
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-200">
          <h3 className="text-lg font-semibold text-slate-900">Edit Owner</h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label htmlFor="aadhar_card" className="block text-sm font-medium text-slate-700 mb-1">
              Aadhar Card Number
            </label>
            <input
              type="text"
          ...
          <div>
            <label htmlFor="pan_card" className="block text-sm font-medium text-slate-700 mb-1">
              PAN Card Number
            </label>
            <input
              type="text"
